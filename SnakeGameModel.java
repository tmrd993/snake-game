...
public class SnakeGameModel {
	...
	if ((head.getX() + SnakeGameModel.SNAKE_SIZE_X > (SnakeGameModel.SCREEN_WIDTH + 5)
		|| head.getY() + SnakeGameModel.SNAKE_SIZE_Y > (SnakeGameModel.SCREEN_HEIGHT + 5)) || head.getX() < -5
		|| head.getY() < -5) {
	    return true;
	}
	...
	Rectangle fruit = new Rectangle(getFruitX(), getFruitY(), SnakeGameModel.SNAKE_SIZE_X,
    ...
    // moves the snake 25 units in the current direction, checks gamelogic and
    // notifies observing classes
    public void step() {
	move();
	checkGameLogic();
	notifyObservers();
